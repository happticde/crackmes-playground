...
def unzip_file(zip_filepath: str, password: Optional[str] = None, extract_dir: Optional[str] = None) -> bool:
    """
    ...
    final_extract_dir = os.path.join(base_extract_dir, "crackme") # Always extract to 'crackme' subdirectory
        ...
        with zipfile.ZipFile(zip_filepath, 'r') as zf:
            ...
            for member in zf.namelist():
                ...
                if os.path.isfile(member_path) and member_path.lower().endswith('.zip'):
                    print(f"Found nested zip file: {member_path}. Attempting to unzip recursively.")
                    # Recursively call unzip_file, passing the current final_extract_dir as the base for the next 'crackme' dir
                    unzip_file(member_path, password, final_extract_dir) # Pass password for nested zips too

            return True
    except zipfile.BadZipFile:
        print(f"Error: {zip_filepath} is a bad zip file.", file=sys.stderr)
    except RuntimeError as e: # For incorrect password
        print(f"Error unzipping {zip_filepath}: {e}. Incorrect password?", file=sys.stderr)
    except Exception as e:
        print(f"An unexpected error occurred while unzipping {zip_filepath}: {e}", file=sys.stderr)
    return False


def generate_markdown(
    title: str, details: Dict[str, str], description: str
) -> str:
...
def scrape_crackme(crackme_id: str, output_dir: str, password: Optional[str] = None) -> None:
    ...
    safe_title_for_dir = "".join(c for c in safe_title_for_dir if c.isalnum() or c in ("_",)).rstrip()
    ...
    if isinstance(panel_background_div, Tag):
        ...
        for col in detail_columns:
            p_tag = col.find('p')
            if p_tag and p_tag.text.strip().startswith('Author:'): # Handle the explicit Author: line first
                key = 'Author'
    ...
    description_header_p = soup.find('p', string=lambda text: text and "Description" in text and text.strip() == "Description")
    if isinstance(description_header_p, Tag):
        ...
        if isinstance(description_span, Tag):
            ...
            final_description_text = description_span.find('span', style="white-space: pre-line")
            if isinstance(final_description_text, Tag):
                description = final_description_text.text.strip()
            else:
                description = description_span.text.strip() # Fallback if span not found

    # --- Download File ---
    download_link = soup.find('a', class_='btn-download') # Use class for more specific selection
    if isinstance(download_link, Tag):
        ...
        if zip_filepath:
            ...
            if not unzipped:
                print(f"Warning: Could not unzip {zip_filepath} with any provided or default passwords. Trying without password.", file=sys.stderr)
                if not unzip_file(zip_filepath):
                    print(f"Warning: Could not unzip {zip_filepath} without password.", file=sys.stderr)
    else:
    ...
    try:
        ...
        print(f"Error: Could not write to file {md_filename}. Reason: {e}", file=sys.stderr)
        sys.exit(1)
